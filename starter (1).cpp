...
int main() {
    ...
    /*
    get_max_numeric isn`t work in str type, so apple and banana isn`t a numeric type, so it make a type error
    */
    std::cout << std::endl;
...
//    std::cout << "Accumulated floats: " << integer_acc.get_total() << std::endl;
    /*
    accumulator type is only work in integral type, but float type isn`t integral type, so it also make a error
    */
